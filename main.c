...
bool network_check(uint8_t address)
{
    ...
    // send check
    error = network_send(address, NETWORK_STATUS_CHECK, NETWORK_COMMAND_NONE, NULL, 0);
    if (error == NETWORK_NO_ERROR) 
    {
        // wait for check acknowledge
        error = network_wait_for_packet(address, NETWORK_STATUS_ACKNOWLEDGE, NETWORK_COMMAND_NONE, NETWORK_ID_ANY, &packet_header, NULL, NETWORK_TIMEOUT_CHECK);
    }
...
uint8_t network_get_response(uint8_t address, uint8_t command, const uint8_t *request_data, uint8_t request_length, uint8_t **response_data, uint8_t *response_length, int16_t timeout)
{
    uint8_t error = NETWORK_NO_ERROR;
    ...
    // send request
    error = network_send(address, NETWORK_STATUS_REQUEST, command, request_data, request_length);
    if (error == NETWORK_NO_ERROR && address != NETWORK_ADDRESS_BROADCAST) 
    {
        // receive response
        error = network_wait_for_packet(address, NETWORK_STATUS_RESPONSE, command, NETWORK_ID_ANY, &response_packet, response_data, timeout);
...
uint8_t network_get_request(struct network_request_data *request, int16_t timeout)
{
    uint8_t error = NETWORK_NO_ERROR;
    ...
    // receive request
    error = network_wait_for_packet(NETWORK_ADDRESS_NONE, NETWORK_STATUS_REQUEST, NETWORK_COMMAND_NONE, NETWORK_ID_ANY, &request_packet, &request->data, timeout);
    if (error == NETWORK_NO_ERROR) 
    {
...
uint8_t network_send_response(const struct network_request_data *request, const uint8_t *data, uint8_t length)
{
    NETWORK_CHECK_IS_INITIALIZED
    // send response
    return network_send(request->source, NETWORK_STATUS_RESPONSE, request->command, data, length);
}

uint8_t network_send(uint8_t destination, uint8_t status, uint8_t command, const uint8_t *data, uint8_t length)
{
    uint8_t error = NETWORK_NO_ERROR;
    ...
    do 
    {
        ...
        attempts++;
    } while (error == NETWORK_TIMEOUT_EXCEEDED && attempts < NETWORK_SEND_ATTEMPTS);
    return error;
}

uint8_t network_wait_for_acknowledge(const struct network_packet_header *packet)
{
    uint8_t error = NETWORK_NO_ERROR;
    ...
    if (packet->status != NETWORK_STATUS_CHECK && packet->status != NETWORK_STATUS_ACKNOWLEDGE && packet->Status != NETWORK_STATUS_DATA_TO_BIG) {
    error = network_wait_for_packet(packet->destination, NETWORK_STATUS_ACK_OR_DTB, packet->command, packet->id, &acknowledge_packet, NULL, NETWORK_TIMEOUT_ACKNOWLEDGE);
    /*
    if (error == NETWORK_NO_ERROR)
    {
...
uint8_t network_wait_for_packet(uint8_t source, uint8_t status, uint8_t command, uint8_t id, struct network_packet_header *packet_header, uint8_t **packet_data, int16_t timeout)
{
    uint8_t error = NETWORK_NO_ERROR;
    ...
    do 
    {
        ...
        if (error == NETWORK_NO_ERROR) 
        {
            ...
            if (packet_header != NULL && ((source != NETWORK_ADDRESS_NONE && source != packet_header->source) || (status != NETWORK_STATUS_ANY && status != packet_header->status /*&& (status == NETWORK_STATUS_ACK_OR_DTB && packet_header->status != NETWORK_STATUS_ACKNOWLEDGE && packet_header->status != NETWORK_STATUS_DATA_TO_BIG)*/) || (command != NETWORK_COMMAND_NONE && command != packet_header->command) || (id != NETWORK_ID_ANY && id != packet_header->id)))
            {
    ...
    } while (error == NETWORK_NO_DATA && (timeout == NETWORK_TIMEOUT_INFINITE || timeout-- > 0));
...
uint8_t network_get_last_packet(struct network_packet_header *packet_header, uint8_t **packet_data)
{
    uint8_t error = NETWORK_NO_ERROR;
...
uint8_t network_acknowledge_packet(const struct network_packet_header *packet)
{
    ...
    if (packet != NULL && packet->source != NETWORK_ADDRESS_BROADCAST && packet->status != NETWORK_STATUS_ACKNOWLEDGE) 
    {
...
uint8_t network_write_packet(struct network_packet_header *packet_header, const uint8_t *packet_data)
{
    uint8_t error = NETWORK_NO_ERROR;
    uint8_t attempts = 0;
    ...
    do 
    {
        ...
        attempts++;
    } while (error == NETWORK_COLLISION_DETECTED && attempts < NETWORK_WRITE_PACKET_ATTEMPTS);
    return error;
}
...
uint8_t network_process_byte(uint8_t error, uint8_t data)
{
    ...
    if (error == NETWORK_NO_ERROR) 
    {
        if (packet_index < sizeof(struct network_packet_header)) 
        {
            ...
            if (packet_index == 2) 
            {
                ...
                if (packet_header.destination != network_conn.address && packet_header.destination != NETWORK_ADDRESS_BROADCAST) 
                {
                ...
                {
                    network_send(packet_header.source, NETWORK_STATUS_DATA_TO_BIG, NETWORK_COMMAND_NONE, NULL, 0);
                    error = NETWORK_INVALID_PACKET;
                }
                #endif
                */
            ...
            // when header read check checksum
                if (network_calculate_checksum(&packet_header) != packet_header.checksum)
                {
                    error = NETWORK_INVALID_PACKET;
                }
            }
        }
        else if (packet_index - sizeof(struct network_packet_header) < packet_header.length) 
        {
            if (packet_data == NULL)
            {
            ...
            // write byte to data
            packet_data[packet_index++ - sizeof(struct network_packet_header)] = data;
        }
    }

    if (error == NETWORK_NO_ERROR) 
    {
        if (packet_index == sizeof(struct network_packet_header) + packet_header.length) 
        { 
            // packet read
            if (packet_header.status == NETWORK_STATUS_CHECK) 
            {
                // respond to check
                network_send(packet_header.source, NETWORK_STATUS_ACKNOWLEDGE, NETWORK_COMMAND_NONE, NULL, 0);
            } 
            else if (!network_conn.is_packet_avaiable) 
            {
                memcpy(&network_conn.last_packet_header, &packet_header, sizeof(struct network_packet_header));
    ...
    {
        bytes_to_skip = sizeof(struct network_packet_header) + packet_header.length - packet_index;
        reset = true;
    }
...
uint8_t network_calculate_checksum(struct network_packet_header *packet)
{
    ...
    packet->checksum = 0;
    for (current = (uint8_t*)packet; current < (uint8_t*)packet + sizeof(struct network_packet_header); current++)
    {
        checksum ^= *current;
    }
...
void network_write_byte(uint8_t byte)
{
    writeModeByte = byte;
    writeMode = true;
